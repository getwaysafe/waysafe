 ...
 * fail loudly instead of silently when BLES_REQUIRE_STRIPE=1 says there
 * should be one.
 *
...
export function requireStripeOrExplainSkip(suiteName: string, reachable: boolean): void {
  if (reachable) return;
  if (process.env.BLES_REQUIRE_STRIPE !== "1") return;

  describe(suiteName, () => {
    it("requires a configured STRIPE_SECRET_KEY because BLES_REQUIRE_STRIPE=1", () => {
      throw new Error(
        `BLES_REQUIRE_STRIPE=1 but STRIPE_SECRET_KEY is unset or still the placeholder -- ` +
          `refusing to silently skip "${suiteName}". Set a real test-mode key, or unset ` +
          `BLES_REQUIRE_STRIPE to allow skipping.`,
      );
    });
  });
}
