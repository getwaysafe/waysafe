...
describe("InMemoryAgentKeyRepository", () => {
  ...
  it("THE ATTACK: rejects an unknown key", async () => {
    ...
    const result = await repo.verifyKey("ap_live_00000000forgedsecretvaluethatdoesnotexist", NOW);
  ...
  it("does not update lastUsedAt on a failed verification", async () => {
    ...
    await repo.verifyKey("ap_live_00000000wrongsecretentirely", new Date(NOW.getTime() + 60_000));
