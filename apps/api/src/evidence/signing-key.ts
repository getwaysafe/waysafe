 ...
 * `@bles/core`'s evidence-signing.ts is pure key/sign/verify math; loading
 * from the environment, and the ephemeral-key fallback, are the I/O it
 * deliberately doesn't do.
 */

import { generateEvidenceSigningKeyPair, loadEvidenceSigningKey } from "@bles/core";
import type { KeyObject } from "node:crypto";

/**
 * Loads `BLES_EVIDENCE_SIGNING_KEY` if set; otherwise generates a fresh key
 ...
 * generate a real key with `npm run keygen -w @bles/api` and set the env
 * var.
 */
export function loadOrGenerateEvidenceSigningKey(warn?: (message: string) => void): KeyObject {
  const configured = process.env.BLES_EVIDENCE_SIGNING_KEY;
  if (configured) return loadEvidenceSigningKey(configured);

  (warn ?? console.warn)(
    "BLES_EVIDENCE_SIGNING_KEY not set -- generated an ephemeral evidence-signing key for this " +
      "process only. Signatures will not verify after a restart or from a different process. " +
      "Generate a real one with `npm run keygen -w @bles/api`.",
  );
