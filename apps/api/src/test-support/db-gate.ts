 ...
 *    stays green. `WAYSAFE_REQUIRE_DB=1` turns that into a hard failure.
 *
 ...
 * the same suite. If `WAYSAFE_REQUIRE_DB=1` and the database isn't
...
export function requireDbOrExplainSkip(suiteName: string, reachable: boolean): void {
  ...
  describe(suiteName, () => {
    it("requires a reachable database because WAYSAFE_REQUIRE_DB=1", () => {
      throw new Error(
        `WAYSAFE_REQUIRE_DB=1 but DATABASE_URL is unset or unreachable -- ` +
          `refusing to silently skip "${suiteName}". Fix the connection, or ` +
          `unset WAYSAFE_REQUIRE_DB to allow skipping.`,
      );
    });
  });
}
