 ...
 *    stays green. `BLES_REQUIRE_DB=1` turns that into a hard failure.
 *
 ...
 * the same suite. If `BLES_REQUIRE_DB=1` and the database isn't
...
export function requireDbOrExplainSkip(suiteName: string, reachable: boolean): void {
  if (reachable) return;
  if (process.env.BLES_REQUIRE_DB !== "1") return;

  describe(suiteName, () => {
    it("requires a reachable database because BLES_REQUIRE_DB=1", () => {
      throw new Error(
        `BLES_REQUIRE_DB=1 but DATABASE_URL is unset or unreachable -- ` +
          `refusing to silently skip "${suiteName}". Fix the connection, or ` +
          `unset BLES_REQUIRE_DB to allow skipping.`,
      );
    });
  });
}
