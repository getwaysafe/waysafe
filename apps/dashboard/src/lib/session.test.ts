...
const ORIGINAL_SECRET = process.env.AGENTPAY_DASHBOARD_SESSION_SECRET;

beforeEach(() => {
  // A real (if test-only) 32-byte key -- not the app's actual secret.
  process.env.AGENTPAY_DASHBOARD_SESSION_SECRET = Buffer.alloc(32, 7).toString("base64");
});

afterEach(() => {
  process.env.AGENTPAY_DASHBOARD_SESSION_SECRET = ORIGINAL_SECRET;
});

describe("session cookie encryption", () => {
  it("round-trips an org credential", () => {
    const token = encryptSession("ap_live_realorgcredential123");
    expect(decryptSession(token)).toBe("ap_live_realorgcredential123");
  });

  it("never contains the raw credential in its encoded form", () => {
    const token = encryptSession("ap_live_realorgcredential123");
    expect(token).not.toContain("ap_live_realorgcredential123");
  });
  ...
  it("THE ATTACK: a cookie encrypted under a different secret does not decrypt under this one", () => {
    const token = encryptSession("ap_live_realorgcredential123");
    process.env.AGENTPAY_DASHBOARD_SESSION_SECRET = Buffer.alloc(32, 9).toString("base64");
    expect(decryptSession(token)).toBeNull();
  });

  it("throws a clear error when the secret is missing", () => {
    delete process.env.AGENTPAY_DASHBOARD_SESSION_SECRET;
    expect(() => encryptSession("ap_live_x")).toThrow(/AGENTPAY_DASHBOARD_SESSION_SECRET/);
  });

  it("throws a clear error when the secret isn't 32 bytes", () => {
    process.env.AGENTPAY_DASHBOARD_SESSION_SECRET = Buffer.alloc(16, 1).toString("base64");
    expect(() => encryptSession("ap_live_x")).toThrow(/32 bytes/);
  });
});
