...
import { Waysafe, UnauthorizedError } from "@waysafe/sdk";
import { WAYSAFE_API_BASE_URL } from "../../lib/waysafe";
 ...
 * authenticate a real, unrevoked credential against the Waysafe API
...
export async function login(_prevState: LoginState, formData: FormData): Promise<LoginState> {
  ...
  const client = new Waysafe({ baseUrl: WAYSAFE_API_BASE_URL, apiKey });
  try {
    ...
    return {
      error: `Could not reach the Waysafe API at ${WAYSAFE_API_BASE_URL}: ${
