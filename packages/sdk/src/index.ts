...
import type { ChainVerificationResult, EvidenceEvent, EvidenceKeyDirectoryEntry } from "@waysafe/core";
import {
...
export function verifyEvidenceIndependently(
  ...
  keyDirectory?: EvidenceKeyDirectoryEntry[],
): ChainVerificationResult {
  const publicKey = loadEvidencePublicKey(publicKeyBase64);
  const directory = keyDirectory ? loadEvidenceKeyDirectory(keyDirectory) : undefined;
  ...
  return verifyEvidenceChainLocally(asEvidenceEvents, publicKey, directory);
}
