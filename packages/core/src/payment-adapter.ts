 ...
 * detail. The only reason Waysafe is a router and not a wrapper around one
...
export interface RailCapability {
   ...
   * Waysafe-side ledger concept for this rail, not something mirrored by
   ...
   * of when Waysafe records the authorization decision. */
  settlement: "instant" | "delayed";
}

export interface ExecutionRequest {
  /** Waysafe's own id for the authorization being executed. The join key
   ...
   * Waysafe's `idempotency_key` on the authorization request -- this one
...
export type ExecutionResult =
  | {
       ...
       * join key back into Waysafe's records, only stored alongside it. */
